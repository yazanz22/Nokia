...
- Gamma rounds numbers. Confirm **92.2%**, **88.6%** and **95.2%** survived intact.
...
Every figure below traces to `docs/EVIDENCE.md`, `ml/metrics.json` or `ml/baselines.json`
(the last regenerated by `python ml/baselines.py` — re-run it after any retrain).
...
2. Device Roaming Status — the signal nothing else can see. A machine can be healthy,
   ...
   the site boundary at NEOM, an asset can hand off to a Jordanian
