...
long range — **54.2% at 72–96 h against our 7.3%**, with a median lead time of **102 h
against our 72 h**.
...
hours where the model catches **92.2–100%**; it fires at least once on **18 of the 24
never-failing test machines** where the model fires on **0 of 24**; and only **90.0%** of
...
truck. (Every figure in these two paragraphs is `ml/baselines.json`, committed alongside
`metrics.json` and regenerated by `python ml/baselines.py`, which loads the same committed
model, imports the split and the repair-gap rule from `ml/train.py` and `ml/features.py`,
and rate-matches the slope threshold to the model's own alarm budget.)
